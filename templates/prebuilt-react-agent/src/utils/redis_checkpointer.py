...
class AsyncRedisSaver(BaseCheckpointSaver):
    ...
    async def from_conn_info(
        cls, *, host: str, port: int, db: int, max_connections: int = 10
    ) -> AsyncIterator["AsyncRedisSaver"]:
        """Create a Redis saver from connection parameters with connection pooling.
        ...
        Returns:
            AsyncRedisSaver instance with pooled connections
        """
        pool = None
        conn = None
        try:
            ...
            conn = AsyncRedis(connection_pool=pool)
            ...
            if conn:
                await conn.aclose()
            # The pool will be closed when all connections are closed

    @classmethod
    @asynccontextmanager
    async def from_pool(cls, pool: ConnectionPool) -> AsyncIterator["AsyncRedisSaver"]:
        """Create a Redis saver from an existing connection pool.

        Args:
            pool: Redis connection pool

        Returns:
            AsyncRedisSaver instance using the provided connection pool
        """
        conn = None
        try:
            conn = AsyncRedis(connection_pool=pool)
    ...
    async def aclose(self) -> None:
        """Close the Redis connection and pool if owned by this instance."""
        if self.conn:
